...
import type { AvailabilitySlot, DoctorLeave } from '../../types/domain'
...
export default function DoctorAvailabilityPage() {
  ...
  const addSlot = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError((err as ApiError).message || 'Could not add that slot.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const toggleSlot = async (slot: AvailabilitySlot) => {
    setSlots((prev) => (prev ? prev.map((s) => (s.id === slot.id ? { ...s, isActive: !s.isActive } : s)) : prev))
    try {
      await doctorSelf.setAvailabilitySlotActive(slot.id, !slot.isActive)
    } catch {
      setSlots((prev) => (prev ? prev.map((s) => (s.id === slot.id ? { ...s, isActive: slot.isActive } : s)) : prev))
      setError('Could not update that slot.')
    }
  }

  const addLeave = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    setError('')
    setSaving(true)
    try {
      ...
      setError((err as ApiError).message || 'Could not add that leave period.')
    } finally {
      setSaving(false)
    }
  }
