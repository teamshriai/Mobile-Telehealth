...
import type { StaffProfile, Hospital } from '../../types/domain'
import type { ApiError } from '../../types/api'

type Tier = 'required' | 'recommended' | 'optional'
type Mode = 'create' | 'join'

/** The staff profile plus the transient, client-only fields this screen's
 *  "create a new hospital" / "join an existing one" choice needs — neither
 *  is a StaffProfile column, they only ever feed the one-shot create/join
 *  call in finishRequired. */
type OnboardingForm = Partial<StaffProfile> & {
  newHospitalName?: string
  newHospitalCity?: string
  newHospitalState?: string
...
export default function HospitalAdminOnboarding() {
  ...
  const finishRequired = async () => {
    ...
    try {
      ...
      setError((err as ApiError).message || 'Could not set up your hospital. Please try again.')
    } finally {
      setSaving(false)
    }
  }
