...
import { User, Phone, MapPin, Shield, Edit3, X, Check, type LucideIcon } from 'lucide-react'
...
function toFormState(profile: PatientProfile | null): ProfileFormState {
  return {
...
function toUpdatePayload(form: ProfileFormState): ProfileUpdatePayload {
...
export default function Profile() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (apiErr.fieldErrors && !Array.isArray(apiErr.fieldErrors)) {
        const mapped: Record<string, string> = {}
        Object.entries(apiErr.fieldErrors).forEach(([field, messages]) => {
          mapped[field] = Array.isArray(messages) ? messages[0] : messages
        })
        setFieldErrors(mapped)
      }
      setSaveError(apiErr.message || 'Could not save your changes. Please try again.')
    } finally {
      setSaving(false)
    }
  }
...
function bloodGroupLabel(value: string | null): string {
...
function FieldGroup({ title, icon: Icon, last, children }: FieldGroupProps) {
  return (
...
function TextField({ label, name, value, editing, onChange, error, type = 'text', wide, required, placeholder, maxLength, displayValue }: TextFieldProps) {
...
function SelectField({ label, name, value, editing, onChange, options, displayValue }: SelectFieldProps) {
  return (
    <div>
      <label htmlFor={name} className="mb-1 block text-xs font-medium text-ink-subtle">
        {label}
